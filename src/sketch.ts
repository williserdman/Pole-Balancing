...
const GENERATION_TIME = 1000 * 5; // 5 seconds
...
export default function sketch(sk: any) {
    ...
    sk.draw = () => {
        ...
        if (Date.now() - timeTracker >= GENERATION_TIME) {
            restrictDatingPool();
            normalizeFitness();
            reproduction();
    ...
    function restrictDatingPool() {
        ...
        controllers.forEach((c) =>
            topFitnesses.includes(c.fitness) ? topControllers.push(c) : {}
        );
    ...
    function reproduction(): void {
        ...
        let t2: number[] = [];
        controllers.forEach((c) => t2.push(c.fitness));
        //console.log(t2);
        ...
        let temp = new CartController(sk, handImage, sk.height - 120);
        let t3: number[] = [];
        controllers.forEach((c) => t3.push(c.fitness));
